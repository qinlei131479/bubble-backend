<template>
  <div class="layout-padding">
    <div class="layout-padding-auto layout-padding-view">
      ...
      <el-row>
        <div class="mb8" style="width: 100%">
          ...
          <right-toolbar v-model:showSearch="showSearch" :export="'agi_datasourceTable_export'"
      ...
      <el-table :data="state.dataList" v-loading="state.loading" border 
        ...
        <el-table-column label="操作" width="150">
          <template #default="scope">
            <el-button icon="edit-pen" text type="primary" v-auth="'agi_datasourceTable_edit'"
              @click="formDialogRef.openDialog(scope.row.id)">编辑</el-button>
            <el-button icon="delete" text type="primary" v-auth="'agi_datasourceTable_del'" @click="handleDelete([scope.row.id])">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <upload-excel
			ref="excelUploadRef"
			title="导入"
			url="/agi/datasourceTable/import"
      temp-url="/admin/sys-file/local/file/datasourceTable.xlsx"
			@refreshDataList="getDataList"
		/>
  </div>
</template>

<script setup lang="ts" name="systemDatasourceTable">
import { BasicTableProps, useTable } from "/@/hooks/table";
import { fetchList, delObjs } from "/@/api/agi/datasourceTable";
...
// 导出excel
const exportExcel = () => {
  downBlobFile('/agi/datasourceTable/export', Object.assign(state.queryForm, { ids: selectObjs }), 'datasourceTable.xlsx')
}
